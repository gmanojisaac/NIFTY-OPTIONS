...
app.get("/dashboard/state", (_req, res) => {
  ...
  console.log("[DASHBOARD][SERVER] getPaperPositions =", paperPositions);
...
<body>
  ...
  // Expecting format "YYYYMMDDHHmm", e.g. "202512090101"
  if (typeof key !== "string" || key.length !== 12) {
    return key; // fallback – just show raw
  }

  const year   = parseInt(key.slice(0, 4), 10);
  const month  = parseInt(key.slice(4, 6), 10); // 1–12
  const day    = parseInt(key.slice(6, 8), 10);
  const hour   = parseInt(key.slice(8,10), 10);
  const minute = parseInt(key.slice(10,12), 10);

  // Treat minuteKey as UTC time; adjust to IST in toLocaleString
  const d = new Date(Date.UTC(year, month - 1, day, hour, minute, 0));

  // Only show HH:MM in Indian time
...
function renderPaperDebug(debug) {
  ...
  // If debug is an array: [{ symbol, pos, ... }] or [{ symbol, state: {...} }]
  if (Array.isArray(debug)) {
    // keep only entries with a symbol
    ...
    el.innerHTML = entries.map(entry => {
      // support both shapes: either entry is the state itself, or entry.state
  ...
  // If debug is an object map: { [symbol]: state }
...
<body>
  ...
  <!-- NAV: 9 sections -->
  <div class="nav">
    <button class="nav-btn active" data-section="main">Main Page</button>
  ...
  // ---- renderers (same as before, with a small LiveSIM debug one) ----
  function renderLTP(list){
    const el = document.getElementById('ltp-list');
    el.innerHTML = list.map(i => 
      '<div class="row"><div><div>'+i.symbol+
